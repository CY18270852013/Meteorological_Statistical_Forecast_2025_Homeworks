...
weights = weights * np.ones((1, ssta.longitude.size))  # 扩展到(lat, lon)
...
# 空间模态
ax1 = fig.add_axes([0.08, 0.37, 0.84, 0.6], 
...
ax1.set_yticks(np.arange(20, 61, 10), crs=ccrs.PlateCarree())
...
ax1.set_title(f'EOF1 (Variance Explained: {variance:.1f}%)', pad=20)
# 颜色条
...
ax2.set_title('PC1 Time Series', pad=10)
ax2.set_xlabel('Year', labelpad=8)
ax2.set_ylabel('Normalized Units', labelpad=8)

# 保存输出
plt.savefig('Global_warming_analysis.png', dpi=300, bbox_inches='tight')
plt.show()